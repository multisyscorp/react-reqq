...
export const transformIncluded = (x, included) => {
  ...
  _.forOwn(x.relationships, (v, k) => {
    rowIncluded[k] = Array.isArray(v.data) ? v.data.map(z => included.find(y => y.type === _.get(z, 'type') && y.id === _.get(z, 'id')) || {}) : included.find(y => y.type === _.get(v, 'data.type') && y.id === _.get(v, 'data.id')) || {};
  });
...
export default {
  ...
  [c.GOT_LIST]: (state, action) => endLoading({ ...state,
    [action.key]: formatList(state[action.key] || {}, action.res)
  }, action, 'list'),
  [c.GET_SHOW]: (state, action) => startLoading({ ...state,
